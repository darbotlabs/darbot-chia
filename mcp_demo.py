...
def demonstrate_mcp_protocol():
    ...
    tx_request = MCPRequest(
        method="tools/call",
        id=2,
        params={"name": "get_transactions", "arguments": {"wallet_id": 1, "start": 0, "end": 3}},
    )
    ...
    resource_request = MCPRequest(method="resources/read", id=3, params={"uri": "chia://blockchain/status"})
    ...
    error_request = MCPRequest(method="tools/call", id=5, params={"name": "nonexistent_tool", "arguments": {}})

    error_response = MCPResponse(id=5, error={"code": -32601, "message": "Tool not found: nonexistent_tool"})
