...
class TestMCPProtocol:
    ...
    def test_serialize_response(self):
        ...
        expected = {"jsonrpc": "2.0", "id": 1, "result": {"success": True}, "error": None}
...
class TestChiaMCPServer:
    ...
    def test_default_resources_registered(self, server):
        ...
        expected_resources = ["chia://blockchain/status", "chia://wallet/balance", "chia://wallet/transactions"]
    ...
    async def test_tool_call_handler_unknown_tool(self, server):
        ...
        request = MCPRequest(method="tools/call", params={"name": "unknown_tool", "arguments": {}}, id=1)
    ...
    async def test_tool_call_handler_missing_name(self, server):
        ...
        handler = ChiaToolCallHandler(server)
        request = MCPRequest(method="tools/call", params={"arguments": {}}, id=1)
    ...
    async def test_tool_execution_get_wallet_balance(self, mock_rpc_client, server):
        ...
        handler = ChiaToolCallHandler(server)
        request = MCPRequest(
            method="tools/call", params={"name": "get_wallet_balance", "arguments": {"wallet_id": 1}}, id=1
        )
        ...
        mock_client_instance.fetch.assert_called_once_with("get_wallet_balance", {"wallet_id": 1})
...
class TestMCPIntegration:
    ...
    async def test_server_start_stop(self):
        ...
        with (
            patch("chia.mcp.server.load_config") as mock_load_config,
