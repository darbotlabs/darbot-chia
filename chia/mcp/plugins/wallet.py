...
class WalletPlugin(MCPPlugin):
        ...
        self.add_tool(
            MCPTool(
                ...
                input_schema={
                    ...
                    "examples": [{"wallet_id": 1}, {"wallet_id": 2, "include_pending": False}],
                },
            )
        )
        ...
        self.add_tool(
            MCPTool(
                ...
                input_schema={
                    ...
                    "examples": [{"wallet_id": 1, "end": 20}, {"wallet_id": 1, "type_filter": 1, "end": 10}],
                },
            )
        )
        ...
        self.add_tool(
            MCPTool(
                ...
                input_schema={
                    "type": "object",
                    "properties": {
                        "transaction_id": {"type": "string", "description": "Transaction ID to get details for"}
                    },
                    "required": ["transaction_id"],
                    "examples": [{"transaction_id": "0x1a2b3c4d5e6f..."}],
                },
            )
        )
        ...
        self.add_tool(
            MCPTool(
                ...
                input_schema={
                    "type": "object",
                    "properties": {
                        "wallet_id": {"type": "integer", "description": "ID of the wallet to send from", "default": 1},
                        "amount": {
                            "type": "integer",
    ...
    async def _get_wallet_coins(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        coins_response = await self.rpc_client.fetch(
            "get_spendable_coins",
            {
                "wallet_id": wallet_id,
        ...
        return {
            "wallet_id": wallet_id,
            "coins": coins_response.get("confirmed_records", [])
            if confirmed_only
            else coins_response.get("records", []),
            "filter_criteria": {"min_amount": min_amount, "max_amount": max_amount, "confirmed_only": confirmed_only},
            "timestamp": self._get_current_timestamp(),
        }
    ...
    async def _get_wallet_transactions(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tx_response = await self.rpc_client.fetch(
            "get_transactions",
            {
                "wallet_id": wallet_id,
                "start": start,
        ...
        return {
            ...
            "pagination": {"start": start, "end": end, "total_count": len(tx_response.get("transactions", []))},
            "sort_criteria": {"sort_key": sort_key, "reverse": reverse, "type_filter": type_filter},
            "timestamp": self._get_current_timestamp(),
        }
    ...
    async def _send_transaction(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tx_params = {"wallet_id": wallet_id, "amount": amount, "address": address, "fee": fee}
        ...
        return {"transaction": tx_response, "parameters": tx_params, "timestamp": self._get_current_timestamp()}
    ...
    async def _get_wallet_summary(self) -> Dict[str, Any]:
        ...
        summary = {"wallets": [], "total_wallets": 0, "timestamp": self._get_current_timestamp()}
    ...
    async def _get_recent_activity(self) -> Dict[str, Any]:
        ...
        for wallet in wallets_response.get("wallets", []):
            tx_response = await self.rpc_client.fetch(
                "get_transactions", {"wallet_id": wallet["id"], "start": 0, "end": 10}
            )
